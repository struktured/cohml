...
cohmlsh:	$(COBJS) $(MLOBJS)
	ocamlmktop -g -custom -o $@ -ccopt -L$(COH_HOME_CPP)/lib -cclib -lcoherence -cclib -lstdc++ unix.cma $(MLOBJS) $(COBJS)
	mkdir -p log

subscriber:	$(COBJS) $(MLOBJS) subscriber.ml
	ocamlmklib -g -custom -cclib -lstdc++ -cclib -L/usr/lib/x86_64-linux-gnu -L$(COH_HOME_CPP)/lib -cclib -lcoherence unix.cma $(MLOBJS) subscriber.ml $(COBJS)
...
%.o:	%.c
	ocamlc -ccopt -xc++ -ccopt -std=c++11 -ccopt -g3 -c -ccopt -fPIC -o $@ -ccopt -I$(COH_HOME_CPP)/include -cclib -L$(COH_HOME_CPP)/lib -cclib -lstdc++ -cclib -lcoherence -ccopt -DOCAML_VERSION_MINOR=$(OCAML_VERSION_MINOR) $<
